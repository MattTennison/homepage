...
describe('Pexels', () => {
    ...
    describe('#fetchImageInBase64', () => {
        ...
        it('returns the asset as a Base64 string', async () => {
            const assetPath = resolve(__dirname, '../fixtures/pexels/image-assets/dog.webp')
        ...
        it('throws an error if the asset is too large', () => {
            const largeAssetPath = resolve(__dirname, '../fixtures/pexels/image-assets/cat.jpeg')
        ...
        it.each([201, 404, 500])('throws an error if the response status code is %s', (statusCode) => {
            imageScope
                .get('/189349/pexels-photo-189349.jpeg')
                .reply(statusCode);
